...
## 21. Calibration app — Benchmark tab, published-version snapshots, sidebar in execution order — **done, 2026-09-18**

**Status: gate (a)–(e) PASS; (f), Danilo's visual checkpoint, is the one item
still open.** Branch `research/item5-benchmark-tab`. No change to `cyclophaser/`
(verified: empty diff against `develop-v2.1`).

### What the front was for

The calibration produced eleven configurations over two months and there was no
way to see them side by side. Comparing meant loading one into the sidebar,
looking, loading another, and remembering. The tab makes the comparison explicit,
hash-auditable and reproducible.

### Three rules the tab is built around

* **The running code's commit is the provenance.**
  `metadata.cyclophaser_version` reads `2.0.0` in all eleven files and therefore
  distinguishes nothing; `git rev-parse HEAD` is what determines what a column
  shows. Header item 2 is the commit, never the YAML field.
* **`evaluation.bad_cases_count` is never a score.** Those marks were made in
  ...
  record 0, params-9 records 6 — so comparing two of them compares two
  inspections, not two configurations. It is rendered as a labelled historical
  annotation, inside a collapsed expander, and is never a metric.
* **`item19_core.CONFIG` is not repointed.** It pins params-10 deliberately as a
  frozen instrument. `benchmark_core` calls `pair_by_overlap` with the column's
  own output and leaves the module's configuration alone.

The two measurements are always shown together and always named by instrument —
`evaluate_against_labels.py / score_phase_sequences` for the sequence,
`item19_core.pair_by_overlap` (fixed margin 6) for the mature pairing. This
front **declares** which shows what; the general debt between the two
instruments stays open.

Leakage rule, enforced in `benchmark_core.metrics_by_split` rather than left to
the caller: every aggregate is computed over the train split, and aggregates
involving the 16 real test cyclones appear in a separate block labelled TESTE,
never added into the train one.

### Published-version snapshots

`research/snapshots/` — `v1.9.4.json` and `v2.0.0.json`, 63 series each (51 real
+ 12 frozen synthetic), **0 failures**, generated by running each **published**
release in its own isolated virtualenv with that release's package defaults. See
that directory's README for the hashes and the procedure.

**Version 1 is not expressible as a YAML in the current detector** (confirmed
against the wheels, not assumed): 1.9.4 and 2.0.0 have an identical 19-parameter
public signature and neither carries `prominence_relative`, `distance`,
`length_scale`, `mature_method`, `mature_amplitude_fraction`,
`boundary_padding` or any `incipient_*`; the difference is in the code
(`_collapse_plateaux` present in the 2.0.0 wheel, absent from 1.9.4).

**A defect found while doing this, worth carrying forward.** Both isolated venvs
initially imported the **working tree's** `cyclophaser`, not the installed wheel,
because the command ran with the repo root as CWD and `''` precedes
site-packages on `sys.path`. This is the same class as item 13 and it fails
silently — the snapshot would have been of the wrong code with no error. The
generator now refuses to run unless `cyclophaser` resolved inside the running
interpreter's own environment. The lesson generalises: **an isolated environment
is not isolated if the CWD is the repo.**

### The sidebar now follows the detector, not the phase names

Read off `determine_periods.py` and `find_stages.py` rather than assumed:

1 Lanczos → 2 Savgol → 3 `find_peaks_valleys` → 4 intensification → 5 decay →
6 mature → 7 residual → 8 `post_process_periods` (no parameter) → 9 incipient.

Two consequences of reading the real order: the **extrema filter is step 3** — it
runs before every stage and its surviving extrema are what all of them see, so it
moved from near the bottom of the sidebar to above the stage groups — and
**`decay_tail_amplitude_fraction` is read by `find_residual_period`**
(`find_stages.py:588`), not by `find_decay_period`, so it sits under Residual.

Two parameters cross groups and carry a note in their own widget:
`length_scale` (scales the intensification and decay duration thresholds,
`find_stages.py:387`, and changes detected phases on 20160735, 20191014, 20203947
under params-9) and `boundary_padding` (a filter parameter that governs the
incipient: 0/51 series refuse an incipient phase under `reflect`, 33/51 under
`edge`).

### Gate

Declared before running; every number below is the measured one.
...
(c) and (d) both carry **positive controls**: the column test performs the swap
and requires the pin to reject it (and separately asserts the two configurations
are distinguishable at all, so the pin cannot pass vacuously); the sidebar test
was confirmed to fail when a widget key was removed.

### The declared prediction, scored

> `>= 40%` of the 51 real series with a different phase sequence between v1 and
> params-11, with the divergence concentrated at the edges, because of the
> zero-padded filter convolution.

* **Count: CORRECT, and by a wide margin.** 41/51 (**80.4 %**) against v1.9.4,
  40/51 (78.4 %) against v2.0.0 — the threshold was 40 %.
* **Localisation: PARTLY WRONG.** The divergence is concentrated at the
  **leading** edge alone: 60 % of timesteps disagree in the first decile against
  20–36 % elsewhere, and the **last** decile is the quietest region at 20 %.
  "Edges", plural, is not what the data show.
* **Mechanism: NOT ESTABLISHED either way.** The snapshot differs from params-11
  in ~15 parameters simultaneously, so the measurement sizes the gap but
...
**Snapshot provenance re-verified (2026-09-18), on request.** Both committed
files were generated by the single version of the generator that has ever
existed, which carries the isolation guard; each records a `module_file` inside
its own venv's site-packages, not the working tree. Re-running both under the
guard reproduces them **field for field, records included** — the only
difference is the `generated` timestamp. The committed hashes therefore stand
and nothing was regenerated.

**Third positive control added to (c):** the mode leak. Validation is presented
as scored, so an uploaded track surviving in a Validation selection would reach
a run that claims to be scored and is not. The test checks the ordinary path and
then FORCES the id back into the selection — what a stale rerun or a dropped
prune would produce — and requires the guard to reject it again; a companion
...
(`cyclophaser_params-1` … `-11`); only 9, 10 and 11 were there before, which
would have left the tab unable to span the history it exists to show.

### Open / not done

* (f) is unapproved. The front is not closed until Danilo signs off the figures.
* The two-instrument debt is declared, not resolved.
* Out of scope and untouched: `cyclophaser/` logic, the documentation clean-up
  and stale-figure removal (item 6), items 20(b), 20(c), C, D, and re-verifying A.
